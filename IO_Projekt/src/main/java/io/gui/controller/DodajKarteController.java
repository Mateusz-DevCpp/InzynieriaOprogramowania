...
import io.model.system.Uprawnienia;
...
public class DodajKarteController 
{
    private Karta tmp_karta;
    
    @FXML
    public void initialize()
    {
        tmp_karta = new Karta(Main.manager_kart.getMaxID()+1, new Uprawnienia("", "06:00", "14:00"), false);
    ...
    public void zastosujCzas()
    {
        ...
        tmp_karta.getPosiadaneUprawiniena().setGodzinaRozpoczecia(gr);
        tmp_karta.getPosiadaneUprawiniena().setGodzinaZakonczenia(gz);
        
        ta_pos_uprawnienia.setText(tmp_karta.getPosiadaneUprawiniena().toString());
    }
    ...
    public void dodaj()
    {
        ...
        if(tmp_karta.getPosiadaneUprawiniena().getPoziomDostepu().contains(tmp_upr))
        {
            lb_wiadomosc.setVisible(true);
            lb_wiadomosc.setTextFill(Color.web("#FF0000"));
        ...
        {
            tmp_karta.getPosiadaneUprawiniena().addDostep(tmp_upr);
            lb_wiadomosc.setVisible(true);
    ...
    public void usun()
    {
        ...
        if(tmp_karta.getPosiadaneUprawiniena().getPoziomDostepu().contains(tmp_upr))
        {
            tmp_karta.getPosiadaneUprawiniena().rmDostep(tmp_upr);
            lb_wiadomosc.setVisible(true);
            lb_wiadomosc.setTextFill(Color.web("#00FF00"));
    ...
    public void filtruj()
    {
        ...
        if(filtr.length() == 0)
        {
            for(int i=0; i<Main.manager_lokali.count(); i++)
            {
                boolean p = tmp_karta.getPosiadaneUprawiniena().getPoziomDostepu().contains(Main.manager_lokali.get(i).getWymaganeUprawnienia());
    ...
    @FXML 
    public void dodajKarte() throws IOException
    {
        Main.manager_kart.register(tmp_karta);
        mainMenu();
    }
